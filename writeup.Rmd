...
However, it’s important to note that the cluster identifications in our training data were themselves based on groupings that aimed to minimize the multidimensional distances between age, gender, 5k split time, **as well as our explanatory variable, final finish time**. By building OLS regression models on the subset of race results that represents each cluster, and then analyzing the mean error rate across our cluster-specific models on the training data, we run the risk that a runner’s final finish time has effectively “leaked” into our model as a predictor variable through the cluster identification. This could lead to an overly optimistic reduction in mean squared error on the training data that’s not possible to recreate on unseen data where a runner’s cluster is not known in advance.
...
assign.SSE <- function(df){
  SSE <- as.integer(sqrt(sum((predict.finish(df) - df$totaltime)^2) / nrow(df)))
  y.hat = predict(base.mod,new=df)
  SSE2 = as.integer(sqrt(sum((y.hat - df$totaltime)^2) / nrow(df)))
  return (list("clus.mod" = SSE, "base.mod" = SSE))
} 
...
The SSE using clustering and individual regression models is `r SSE`. Using the Boston Marathon baseline linear regression is `r SSE2`. So this model does not generalize well to a marathon with a different profile. 
