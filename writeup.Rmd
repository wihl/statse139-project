...
Using $k$-Fold cross validation, and taking the sum of squares error for each fold resulted in a baseline mean score of `r as.integer(score.mean)` or approximately `r as.integer(sqrt(score.mean))` minutes.
While $\pm$ `r as.integer(sqrt(score.mean))` minutes over the course of a four hour race may not seem like a large error,
$\pm$ `r as.integer(sqrt(score.mean))` minutes could be the difference between qualifying and not qualifying for a competitive race such as the Boston Marathon. 
...
Using $k$-Fold cross validation, and taking the sum of squares error for each fold resulted in a log-transformed mean score of `r as.integer(score.mean)` or approximately `r as.integer(sqrt(score.mean))` minutes, a negligible improvement.
...
We also attempted a range of polynomial transformations on the 5K times using an exponent in the range [1.0,2.5]. However, this did not improve model’s accuracy significantly. We have omitted these results in order to be concise within the report, but they are available on our project GitHub site (See Appendix B).
