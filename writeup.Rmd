...
As the histogram reveals, there were several outliers in the 5k split times. These could be due to timing errors, or an injury or accident at the start of the race. Due to the sensitivity of OLS regression, we elected to remove the outliers that were more than three standard deviations greater than the mean 5k split time. In total there were `r outliers5ksum` rows like this out of 
`r dfm.rows` (`r percent(outliers5ksum / as.numeric(dfm.rows))`).

Age was in the range [18,81]. (The minimum age to run the Boston Marathon is 18). No outliers were removed. Males represented `r percent(male pct)` of total runners, which seemed reasonable.
