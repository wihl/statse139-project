...
Due to this limitation, the B.A.A. has to be very selective in the choice of runners and has established restrictive qualifying times ([source](http://www.baa.org/Races/Boston-Marathon/Participant-Information/Qualifying.aspx)). In order to run
the Boston Marathon, not only does an applicant need to run a previous qualifying marathon, but must also finish
...
rolling hills, especally between miles 16-21, which are among the most difficult of the course. The peak is affectionately known
as Heartbreak Hill.
...
Given the right tail distribution of the QQplot, the fan out of the residuals vs. predicted, and the slight right skewness of the finish time histogram,
we elected to perform a log transformation of the response variable.
...
Using $k$-Fold cross validation, and taking the sum of squares error for each fold resulted in a log-transformed mean score of `r as.integer(score.mean)`
or approximately `r as.integer(sqrt(score.mean))` minutes, a neglible improvement.

$R^2$ went from `r summary(base.mod)$r.squared` to `r summary(tx.mod)$r.squared`, a slight improvement. Adjusted $R^2$ improved by an equally small amount.

We also attempted a range of polynominal transformations of the 5K time using an exponent in the range [1.0,2.5] but this did not improve model accuracy
significantly. For report brevity, these results have been omitted but are available upon request (and can be found in the project GitHub site - See Appendix B).
...
Using the [elbow method](https://en.wikipedia.org/wiki/Determining_the_number_of_clusters_in_a_data_set), we determined that the optimal number
of clusters is $k=8$, which results in a cross validation error of `r as.integer(overallclustererror[6])` or $\pm$ 
...
TODO: As you can see, the unsupervised learning found groupings that would have difficult if not
...
[fpc](https://cran.r-project.org/web/packages/fpc/fpc.pdf) used to plot clusters
